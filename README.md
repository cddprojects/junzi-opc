...
1. **线下工作坊** — 展示位置选「线下工作坊」，上传海报。不关联商品：`/workshop` 只显示海报。关联商品：该页显示商品列表（可保留海报）。
2. **活动报名** — 位置选「活动报名」，规则同上，前台 `/events`。
3. **会员中心** — 位置选「会员中心」，上传海报，并在「详情图」加长图。只海报：`/member` 显示海报 + 详情图画廊。关联「年度会员」：保留开通/续费购买。
4. **AI工具小程序** — 位置选「AI工具小程序」，上传横幅。无图则首页该块隐藏（不再用 CSS 假图）。「图上按钮文案」+「跳转 / 图上按钮链接」画在图上，点进 `/tools` 或你填的地址。

种子图：`public/posters/workshop-sm.jpg`、`events-sm.jpg`、`ai-tool-sm.jpg`。生产库需执行 `supabase/migrations/20260910_000002_poster_placements.sql` 与 `20260910_000003_poster_ai_tools.sql`。
