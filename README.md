...
后台 → **内容 → 海报 / 轮播**：新增或编辑时用「展示位置」下拉选择，不要拆成独立后台页。

1. **线下工作坊** — 位置选「线下工作坊」，上传海报。不关联商品：`/workshop` 只显示海报。关联商品：该页显示商品列表。
2. **活动报名** — 位置选「活动报名」，规则同上，前台 `/events`。
3. **会员中心** — 位置选「会员中心」，上传海报，并在「详情图」加长图。只海报：`/member` 显示海报 + 图廊。关联「年度会员」：保留开通/续费购买。
4. **AI工具小程序** — 位置选「AI工具小程序」。无图则首页该块隐藏。「图上按钮文案」+「跳转 / 图上按钮链接」画在图上。

种子图：`public/posters/workshop-sm.jpg`、`events-sm.jpg`、`ai-tool-sm.jpg`、`member-detail-1.jpg`。生产库需执行 `supabase/migrations/20260910_000002_poster_placements.sql` 与 `20260910_000003_poster_ai_tools.sql`，再运行 `npx tsx scripts/seed-slot-posters.ts`。
...
- 未上传视频时使用占位封面，不复制原片。
- 购物车只存在当前浏览器会话。
- 钱包分桶：充值余额 / 佣金余额 / 提现占用 / 可提现 / 合计。充值不可提现，充值本身不计推荐佣金。购课若走钱包，只扣充值余额，不会动用佣金。提现：待审核 → 已通过 → 已打款；通过不等于已打款。
