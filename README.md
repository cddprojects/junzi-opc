...
「内容」分组有三个固定页；首页轮播 / 中部横幅 / AI 仍在 **海报 / 轮播**（展示位置下拉）。

1. **线下工作坊**（`/admin/workshop`）— 上传海报、跳转、关联商品。不关联：`/workshop` 只显示海报。关联了：显示商品列表。
2. **活动报名**（`/admin/events`）— 同上，前台 `/events`。
3. **会员中心**（`/admin/member`）— 海报 + 详情长图。不关联商品只展示海报/图廊；关联「年度会员」保留开通购买。
4. **AI工具小程序** — 仍在 **海报 / 轮播**。无图则首页隐藏。「图上按钮文案」+ 跳转链接。
